...
static int rtk_fan_ctrl_speed(struct rtk_fan_data *fan_data, int ctrl_speed) {
	...
	if (fan_data->pwm) {
		...
		dev_info(fan_data->dev, "%s to set pwm default duty_ns =%d,  period_ns = %d!",
...
static int rtk_fan_init(struct rtk_fan_data *fan_data, struct platform_device *pdev) {
	 ...
	 * the atomic PWM API.
	 */
	pwm_apply_args(fan_data->pwm);

	pwm_get_args(fan_data->pwm, &pargs);

	fan_data->pwm_period = pargs.period;
	
	/* Enable the PWM */
	pwm_config(fan_data->pwm, fan_data->pwm_period / 2, fan_data->pwm_period);
