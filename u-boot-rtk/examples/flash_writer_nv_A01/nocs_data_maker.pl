...
if( $#ARGV != -1 )
{
	...
	#size_of of auxcode area will be fixed to 64KB
	my $auxcode_file_size = -s $auxcode_file_path;
	if ($auxcode_file_size > $size_of_scs_auxcode_area - 0x220){
		printf STDOUT "ERROR: illegal auxcode_file_size value, need to be <= (size_of_scs_auxcode_area - 0x220) (now auxcode_file_size=0x%x)\n",$auxcode_file_size; 
		exit(-1);
	}
	my $auxcode_padding_size = ($size_of_scs_auxcode_area - 0x220) - $auxcode_file_size;
	open(my $FD_AUXCODE_PADDING, '>:raw', 'Bind/AUXCODE_PADDING.bin') or die "Unable to open: $!";
	for (my $i=0; $i < $auxcode_padding_size; $i++) {
		print $FD_AUXCODE_PADDING pack('C',0xFF) ;
	}
	close($FD_AUXCODE_PADDING);
	
	open(my $FD_SCS_Auxcode_Area, '>:raw', 'Bind/Auxcode_Area.bin.tmp') or die "Unable to open: $!";
	open(my $FD_Auxcode_Nagra_Control_Data, '>:raw', 'Bind/Auxcode_Area.bin.tmp2') or die "Unable to open: $!";
	#tmp is RTK control data
	print $FD_SCS_Auxcode_Area pack('L<',0x1) ;
	print $FD_SCS_Auxcode_Area pack('L<',$teeos_version_ext);
	print $FD_SCS_Auxcode_Area pack('L<',$teeos_mask_vers);
	print $FD_SCS_Auxcode_Area pack('L<',0xFFFFFFFF) ; #padding 4B
	close($FD_SCS_Auxcode_Area);
	#tmp2 is Nagra Control data
	print $FD_Auxcode_Nagra_Control_Data pack('L<',$segmentid_ext);
	print $FD_Auxcode_Nagra_Control_Data pack('L<',$mask_msid);
	print $FD_Auxcode_Nagra_Control_Data pack('L<',$version_ext);
	print $FD_Auxcode_Nagra_Control_Data pack('L<',$mask_vers);
	close($FD_Auxcode_Nagra_Control_Data);
	
	$cmd = "cat " . $auxcode_file_path . " Bind/AUXCODE_PADDING.bin Bind/Auxcode_Area.bin.tmp > Bind/Auxcode_Area.bin";
	system($cmd);
	#save auxcode.bin for AES encryption
	$cmd = "cat " . $auxcode_file_path . " Bind/AUXCODE_PADDING.bin  > Bind/auxcode.bin";
	system($cmd);
	...
	print $FD_PCPU_Code_Area pack('L<',$pcpu_fw_mask_msid);
	print $FD_PCPU_Code_Area pack('L<',$pcpu_fw_version_ext);
	print $FD_PCPU_Code_Area pack('L<',$pcpu_fw_mask_vers);
	print $FD_PCPU_Code_Area pack('L<',0x16151615);
	for (my $i=0; $i < 0x28; $i++) {
		print $FD_PCPU_Code_Area pack('C',0xFF) ;
	}
	...
	if ($pcpu_code_file_size < 0x6ec0){
		...
		for (my $i=0; $i < $padding_len; $i++) {
			print $FD_PCPU_FW_PADDING pack('C',0xFF) ;
		}
