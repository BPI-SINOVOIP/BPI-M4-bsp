...
static int rtk_cpu_hlb_cb(struct notifier_block *nb, unsigned long action, void *p)
{
...
static void rtk_cpu_hlb_load_worker(struct work_struct *work)
{
	...
	queue_delayed_work(system_freezable_wq, &data->dwork, msecs_to_jiffies(data->polling_time));
}
...
static int __init rtk_cpu_hlb_init(void)
{
	...
	rtk_cpu_hlb_pdev = platform_device_register_simple("rtk-cpu-hlb", 0, NULL, 0);
