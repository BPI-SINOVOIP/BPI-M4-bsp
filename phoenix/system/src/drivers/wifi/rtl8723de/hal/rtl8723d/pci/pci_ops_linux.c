...
#ifdef CONFIG_FILL_TXBD1
	/* +128 bytes offset to TXBD1, and +1 for TXBD1 */
	pxmitpriv->txringcount[BCN_QUEUE_INX] =
			DIV_ROUND_UP(128, sizeof(struct tx_buf_desc)) + 1;
#else
...
#ifdef USING_RX_TAG
	rx_tag = (u16)GET_RX_BUFFER_DESC_RX_TAG_8723D(rx_bufdesc);
#else
	total_len = (u16)GET_RX_BUFFER_DESC_TOTAL_LENGTH_8723D(rx_bufdesc);
#endif
	first_seg = (u8)GET_RX_BUFFER_DESC_FS_8723D(rx_bufdesc);
	last_seg = (u8)GET_RX_BUFFER_DESC_LS_8723D(rx_bufdesc);
