 ...
 */
static u16 hal_mdio_read_8723de(
		PADAPTER	Adapter,
		u8		Addr)
{
	u16 ret = 0;
 ...
 *	Created by Roger, 2013.05.08.
 */
static void hal_mdio_write_8723de(
		PADAPTER	Adapter,
		u8		Addr,
		u16		Data)
{
...
static u8 ReadAdapterInfo8723DE(PADAPTER Adapter)
{
...
void rtl8723de_interface_configure(PADAPTER Adapter)
{
	...
	if (!IS_A_CUT(pHalData->version_id) && !IS_B_CUT(pHalData->version_id)) {
		val_16 = hal_mdio_read_8723de(Adapter, 0x9);
		hal_mdio_write_8723de(Adapter, 0x9, val_16 & (~BIT14));
	}

	/*test 0x8[15:12] = 2*/
	val_16 = hal_mdio_read_8723de(Adapter, 0x8);
	val_16 &= 0x0FFF;
	val_16 |= (ClkDelayCount << 12);
	/*val_16 |=  0x2000;*/
	/*val_16 &= 0x2fff;*/
	hal_mdio_write_8723de(Adapter, 0x8, val_16);
...
)
{
	...
	/* Set retry limit */
	rtw_write16(Adapter, REG_RETRY_LIMIT, BIT_SRL(RL_VAL_AP) | BIT_LRL(RL_VAL_AP));
	...
	rtw_write16(Adapter, REG_BCN_CTRL, ((DIS_TSF_UDT | EN_BCN_FUNCTION) << 8) | DIS_TSF_UDT | EN_BCN_FUNCTION);	/* For 2 PORT TSF SYNC */
#else
	rtw_write8(Adapter, REG_BCN_CTRL, DIS_TSF_UDT | EN_BCN_FUNCTION | EN_TXBCN_RPT | DIS_ATIM);
#endif
	...
	phy_set_bb_reg(Adapter, 0x834, BIT(26), 0x0); */
	/*#endif*/
...
{
	u8			i, numNQ = 0, numPubQ = 0, numHQ = 0, numLQ = 0, numEQ = 0;
	/* u8*			numPageArray; */
	...
	if (Adapter->registrypriv.wifi_spec) {
		...
		numEQ = WMM_NORMAL_PAGE_NUM_EPQ_8723D;	/* 0x04 */
	} else {
		numHQ = NORMAL_PAGE_NUM_HPQ_8723D;	/* 0x0c */
		numLQ = NORMAL_PAGE_NUM_LPQ_8723D;	/* 0x02 */
		numNQ = NORMAL_PAGE_NUM_NPQ_8723D;	/* 0x02 */
		numEQ = NORMAL_PAGE_NUM_EPQ_8723D;	/* 0x00 */
	}

	numPubQ = TX_TOTAL_PAGE_NUMBER_8723D - numHQ - numLQ - numNQ - numEQ;
...
ResetPcieInterfaceDMA8723DE(
		PADAPTER		Adapter,
		BOOLEAN		bInMACPowerOn
)
{
	u8	u1Tmp;
	BOOLEAN	bReleaseMACRxPause;
	u8	BackUpPcieDMAPause;
...
static u32 rtl8723de_hal_init(PADAPTER Adapter)
{
	...
	*/
	...
	/* Configure TX HIQ constraint */
	rtw_write8(Adapter, 0x5a7, rtw_read8(Adapter, 0x5a7) | 0x01);
...
#if (defined(CONFIG_ANT_DETECTION))
	odm_single_dual_antenna_default_setting(pDM_Odm);
#endif
	...
	*/
	...
	phy_set_bb_reg(Adapter, rOFDM0_XAAGCCore1, bMaskByte0, 0x50);
	...
	if (Adapter->registrypriv.wifi_spec == 1)
		phy_set_bb_reg(Adapter, rOFDM0_ECCAThreshold,
...
)
{
	...
	if ((rtw_read8(Adapter, REG_MCUFWDL) & RAM_DL_SEL) && GET_HAL_DATA(Adapter)->bFWReady) /* 8051 RAM code */
...
static u32 rtl8723de_hal_deinit(PADAPTER Adapter)
{
	...
	if (bSupportRemoteWakeUp && Adapter->bEnterPnpSleep) {
		...
		if (pMgntInfo->mAssoc &&
			...
			Adapter->hal_func.set_hw_reg_handler(Adapter, HW_VAR_FW_LPS_ACTION, (u8 *)(&bEnterFwLPS));
...
u8 SetHwReg8723DE(PADAPTER Adapter, u8 variable, u8 *val)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
	u8 ret = _SUCCESS;
	...
	case HW_VAR_SET_RPWM:
#ifdef CONFIG_LPS_LCLK
		{
			...
			rtw_write8(Adapter, REG_PCIE_HRPWM_8723DE, ps_state);
		}
#endif
		break;
	...
	case HW_VAR_DBI:
	{
		...
		pCmd = (u16 *)val;
		hal_dbi_write_8723de(Adapter, pCmd[0], (u8)pCmd[1]);
	...
	case HW_VAR_MDIO:
	{
		...
		pCmd = (u16 *)val;
		hal_mdio_write_8723de(Adapter, (u8)pCmd[0], pCmd[1]);
		break;
	}
	default:
		ret = SetHwReg8723D(Adapter, variable, val);
		break;
	}
...
void GetHwReg8723DE(PADAPTER Adapter, u8 variable, u8 *val)
{
	...
	switch (variable) {
	case HW_VAR_DBI:
		*val = hal_dbi_read_8723de(Adapter, *((u16 *)(val)));
		break;
	case HW_VAR_MDIO:
		*((u16 *)(val)) = hal_mdio_read_8723de(Adapter, *val);
		break;
	case HW_VAR_L1OFF_NIC_SUPPORT:
		{
		u8 l1off;

		l1off = hal_dbi_read_8723de(Adapter, 0x188);
		if (l1off & (BIT2|BIT3))
			*val = _TRUE;
		else
			*val = _FALSE;
	...
	case HW_VAR_L1OFF_CAPABILITY:
		...
		l1off = hal_dbi_read_8723de(Adapter, 0x184);
		if (l1off & (BIT2|BIT3))
			*val = _TRUE;
		else
			*val = _FALSE;
...
void rtl8723de_set_hal_ops(_adapter *padapter)
{
	...
	pHalFunc->set_hw_reg_handler = &SetHwReg8723DE;
	pHalFunc->GetHwRegHandler = &GetHwReg8723DE;
	pHalFunc->get_hal_def_var_handler = &GetHalDefVar8723DE;
	...
	pHalFunc->unmap_beacon_icf = &rtl8723de_unmap_beacon_icf;
