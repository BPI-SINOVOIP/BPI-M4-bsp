...
struct xmit_frame *__rtw_alloc_cmdxmitframe_8723de(struct xmit_priv *pxmitpriv,
		enum cmdbuf_type buf_type)
{
	...
	ptxbufdesc = get_txbufdesc(padapter, BCN_QUEUE_INX);

	/* set OWN bit in Beacon tx descriptor */
	if (ptxbufdesc != NULL)
		SET_TX_BUFF_DESC_OWN_8723D(ptxbufdesc, 0);
	else
		return NULL;
...
void fill_txbufdesc_own(_adapter *padapter, u8 *txbufdesc, u16 queue_idx, struct rtw_tx_ring *ptxring)
{
	...
	if (queue_idx == BCN_QUEUE_INX) {
		...
		SET_TX_BUFF_DESC_OWN_8723D(txbufdesc + 128, 1);
#endif
...
s32 rtw_dump_xframe(_adapter *padapter, struct xmit_frame *pxmitframe)
{
	...
	for (t = 0; t < pattrib->nr_frags; t++) {
		...
		if (ff_hwaddr == BCN_QUEUE_INX)
			memcpy(ptxbufdesc + 128, ptxbufdesc, sizeof(struct tx_buf_desc));
#endif
